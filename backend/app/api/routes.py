...
async def get_captions(session_id: str):
    ...
    # Also expose captions from cached dialogue-only translations
    if va_state and va_state.caption_translations:
        for lang_code, translated_script in va_state.caption_translations.items():
            if lang_code not in captions:
                captions[lang_code] = [
                    {
                        "segment_id": seg.segment_index,
                        "text": "\n".join(_extract_quoted_dialogue_lines(seg.veo_prompt)),
...
class EnsureCaptionsRequest(BaseModel):
    """Request schema for on-demand caption translation."""
    session_id: str
    language_codes: List[str] = Field(
        ..., description="Language codes to ensure captions for (e.g. ['bm','zh'])"
    )
    source_language_code: Optional[str] = Field(
        default=None,
        description="Source language to translate from. Defaults to current_language or 'en'."
    )


@router.post("/captions/ensure")
async def ensure_captions(request: EnsureCaptionsRequest):
    """
    Ensure caption text exists for selected languages without regenerating clips.
    Translates dialogue on-demand for languages not already in video_inputs.
    """
    pipeline = _sessions.get(request.session_id)
    if not pipeline:
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    va_state = pipeline.state.visual_audio
    if not va_state or not va_state.veo_script:
        raise HTTPException(status_code=400, detail="No VeoScript available. Generate video assets first.")
    ...
    # Determine source script for translations
    source_script = va_state.original_veo_script or va_state.veo_script
    source_lang = request.source_language_code or va_state.current_language or "en"

    va_agent = pipeline.visual_audio_agent
    va_agent._ensure_client()

    for lang_code in request.language_codes:
        # Skip if already available in video_inputs
        if vp and lang_code in vp.video_inputs:
            continue
        # Skip if already cached
        if lang_code in va_state.caption_translations:
            continue
        # Skip if it's the current language (already in veo_script)
        if lang_code == va_state.current_language:
            continue
        # Skip source language (already available via veo_script)
        if lang_code == source_lang:
            continue

        try:
            logger.info("[CAPTIONS-ENSURE] Translating dialogue to '%s' from '%s'", lang_code, source_lang)
            translated = await va_agent.translate_veo_script_dialogue(source_script, lang_code)
            va_state.caption_translations[lang_code] = translated
            logger.info("[CAPTIONS-ENSURE] Cached translation for '%s' (%d segments)", lang_code, len(translated.segments))
        except Exception as e:
            logger.error("[CAPTIONS-ENSURE] Translation to '%s' failed: %s", lang_code, e, exc_info=True)
            raise HTTPException(status_code=500, detail=f"Caption translation to '{lang_code}' failed: {str(e)}")

    # Build and return full caption map (same as GET /captions)
    def _normalize_caption_text(text: str) -> str:
        quoted = _extract_quoted_dialogue_lines(text or "")
        if quoted:
            return "\n".join(quoted)
    ...
    if vp and vp.video_inputs:
        for lc, vi in vp.video_inputs.items():
            captions[lc] = [
                {"segment_id": s.scene_id, "text": _normalize_caption_text(s.audio_script or "")}
                for s in vi.scenes
            ]

    if va_state.current_language and va_state.current_language not in captions:
        captions[va_state.current_language] = [
            {"segment_id": seg.segment_index, "text": "\n".join(_extract_quoted_dialogue_lines(seg.veo_prompt))}
            for seg in va_state.veo_script.segments
        ]

    for lc, ts in va_state.caption_translations.items():
        if lc not in captions:
            captions[lc] = [
                {"segment_id": seg.segment_index, "text": "\n".join(_extract_quoted_dialogue_lines(seg.veo_prompt))}
                for seg in ts.segments
            ]

    return {"session_id": request.session_id, "captions": captions}
            ...
            for clip in sorted(va_state.veo_clips, key=lambda c: c.segment_index):
                ...
                for lang_code in request.caption_languages:
                    ...
                    if vi:
                        ...
                        va_state.caption_translations
                        and lang_code in va_state.caption_translations
                    ):
                        translated_script = va_state.caption_translations[lang_code]
                        seg = next(
                            (s for s in translated_script.segments if s.segment_index == seg_idx),
                            None,
                        )
                        if seg:
...
# ==================== DEMO SUBTITLE BURN-IN ====================

class BurnSubtitlesRequest(BaseModel):
    """Request schema for burning subtitles into a video from URL + SRT text."""
    video_url: str = Field(..., description="Public URL of the source video to subtitle (e.g. /demo-assets/veoplz_subtitled_source.mp4)")
    srt_content: str = Field(..., description="SRT subtitle content to burn into the video")


@router.post("/export/burn-subtitles")
async def burn_subtitles_into_video(request: BurnSubtitlesRequest):
    """
    Burn SRT subtitles into a video file and return the result.
    Used by demo mode to burn zh/ta captions into pre-stitched demo videos.
    """
    import shutil
    import subprocess
    import tempfile
    ...
    # Resolve video source: handle both local file paths and relative URLs
    video_path_str = request.video_url
    # Support relative paths like /demo-assets/... by looking in frontend/public
    if video_path_str.startswith("/demo-assets/") or video_path_str.startswith("/demo-assets\\"):
        rel = video_path_str.lstrip("/")
        candidates = [
            Path(__file__).resolve().parents[2] / ".." / "frontend" / "public" / rel,
            Path.cwd() / ".." / "frontend" / "public" / rel,
            Path.cwd() / "frontend" / "public" / rel,
        ]
        resolved = None
        for c in candidates:
            if c.resolve().exists():
                ...
                break
        if not resolved:
            raise HTTPException(status_code=400, detail=f"Demo video not found: {video_path_str}")
        video_path_str = resolved

    if not Path(video_path_str).exists():
        raise HTTPException(status_code=400, detail=f"Video file not found: {video_path_str}")

    # Write SRT to temp file
    srt_tmp = tempfile.NamedTemporaryFile(mode="w", suffix=".srt", delete=False, encoding="utf-8")
    srt_path = srt_tmp.name
    try:
        srt_tmp.write(request.srt_content)
    finally:
        srt_tmp.close()
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning("[BURN-SUBS] ffmpeg failed: %s", result.stderr[:2000])
        ...
        return StreamingResponse(
            iterfile(),
            media_type="video/mp4",
            headers={"Content-Disposition": 'attachment; filename="scam-shield-captioned.mp4"'},
        )
    except HTTPException:
        raise
    except Exception as e:
        logger.error("[BURN-SUBS] Failed: %s", e, exc_info=True)
        for p in [srt_path, output_path]:
            try:
                os.unlink(p)
            except OSError:
                pass
        raise HTTPException(status_code=500, detail=f"Subtitle burn-in failed: {str(e)}")
...
class YouTubePublishRequest(BaseModel):
    """Request schema for publishing video directly to YouTube."""
    session_id: str
    title: str = Field(..., min_length=1, max_length=100, description="Video title")
    description: str = Field("", max_length=5000, description="Video description with hashtags")
    tags: List[str] = Field(default_factory=list, description="YouTube tags")
    privacy: str = Field("unlisted", pattern="^(public|unlisted|private)$")
    caption_languages: List[str] = Field(default_factory=list, description="Caption languages to burn in")


class YouTubePublishResponse(BaseModel):
    """Response schema for YouTube publish."""
    session_id: str
    status: str
...
@router.post("/social/publish/youtube", response_model=YouTubePublishResponse)
async def publish_to_youtube(request: YouTubePublishRequest):
    """
    Export the stitched video and upload it directly to YouTube.
    
    Requires YOUTUBE_CLIENT_ID, YOUTUBE_CLIENT_SECRET, and YOUTUBE_REFRESH_TOKEN
    to be set in .env (run scripts/youtube_auth.py once to get the refresh token).
    
    The video is uploaded as unlisted by default (safe for demos).
    ...
    if not all([yt_client_id, yt_client_secret, yt_refresh_token]):
        raise HTTPException(
            status_code=503,
            detail="YouTube credentials not configured. Run scripts/youtube_auth.py first."
        )
    ...
    try:
        ...
        t0 = time.time()
        logger.info("[YOUTUBE] === Starting YouTube publish ===")
        logger.info("[YOUTUBE] Session=%s | Title=%s | Privacy=%s",
                    request.session_id, request.title, request.privacy)
        
        # Step 1: Get the video file — real session or demo fallback
        logger.info("[YOUTUBE] Step 1: Resolving video source...")
        pipeline = _sessions.get(request.session_id)
        video_path: Optional[str] = None
        _video_is_tmp = False  # whether we own the temp file and must delete it

        if pipeline:
            # Real session — export and stream to temp file
            export_req = ExportVideoRequest(
                session_id=request.session_id,
                caption_languages=request.caption_languages,
            )
            export_response = await export_stitched_video(export_req)
            ...
            try:
                async for chunk in export_response.body_iterator:
                    if isinstance(chunk, bytes):
                        ...
                        video_tmp.write(chunk.encode("utf-8"))
            finally:
                video_tmp.close()
        else:
            # Demo / mock session — use pre-built demo video directly
            logger.info("[YOUTUBE] Session not in memory — using demo video fallback")
            # Prefer clean no-caption base; fall back to subtitled source
            demo_filenames = ["veoplz_eng.MP4", "veoplz_eng.mp4", "veoplz_subtitled_source.mp4"]
            demo_dirs = [
                Path(__file__).resolve().parents[3] / "frontend" / "public" / "demo-assets",
                Path.cwd().parent / "frontend" / "public" / "demo-assets",
                Path.cwd() / ".." / "frontend" / "public" / "demo-assets",
            ]
            for demo_dir in demo_dirs:
                for fname in demo_filenames:
                    candidate = (demo_dir / fname).resolve()
                    if candidate.exists():
                ...
                if video_path:
                    break
            if not video_path:
                raise HTTPException(status_code=404, detail="Demo video not found on server. Place veoplz_eng.MP4 in frontend/public/demo-assets/")

        file_size = Path(video_path).stat().st_size
        logger.info("[YOUTUBE] Step 1: Video resolved (%d bytes) path=%s", file_size, video_path)
        
        # Step 2: Authenticate with YouTube API using refresh token
        logger.info("[YOUTUBE] Step 2: Authenticating with YouTube API...")
        ...
        # Step 3: Upload video
        logger.info("[YOUTUBE] Step 3: Uploading to YouTube...")
        body = {
            "snippet": {
                "title": request.title,
                "description": request.description,
                "tags": request.tags[:30],  # YouTube max 30 tags
        ...
        elapsed = time.time() - t0
        logger.info("[YOUTUBE] === Published successfully in %.1fs === URL: %s", elapsed, video_url)
        ...
        return YouTubePublishResponse(
            ...
            video_id=video_id,
            video_url=video_url,
            message=f"Video published to YouTube ({request.privacy})",
        )
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error("[YOUTUBE] Failed to publish: %s", e, exc_info=True)
        # Cleanup temp file on error
        try:
            if '_video_is_tmp' in dir() and _video_is_tmp and 'video_path' in dir() and video_path:
                os.unlink(video_path)
...
class InstagramPublishRequest(BaseModel):
    """Request schema for publishing video as Instagram Reel."""
    session_id: str
    caption: str = Field("", max_length=2200, description="Reel caption with hashtags")
    caption_languages: List[str] = Field(default_factory=list, description="Caption languages to burn in")


class InstagramPublishResponse(BaseModel):
    """Response schema for Instagram publish."""
    session_id: str
    status: str
...
@router.post("/social/publish/instagram", response_model=InstagramPublishResponse)
async def publish_to_instagram(request: InstagramPublishRequest):
    """
    Upload the stitched video as an Instagram Reel via the Instagram Graph API.

    Flow:
      1. Resolve video file (real session or demo fallback)
      2. Upload video to a GCS bucket to get a public URL (Instagram needs a fetchable URL)
      3. Create an IG media container (type REELS)
      4. Poll until the container is ready
      5. Publish the container
      6. Delete the temp GCS object
    """
    import asyncio
    ...
    if not all([ig_access_token, ig_user_id]):
        raise HTTPException(
            status_code=503,
            detail="Instagram credentials not configured. Set INSTAGRAM_ACCESS_TOKEN and INSTAGRAM_USER_ID in .env",
        )

    try:
        import tempfile
        t0 = time.time()
        logger.info("[INSTAGRAM] === Starting Instagram Reels publish ===")
        logger.info("[INSTAGRAM] Session=%s", request.session_id)
        ...
        if pipeline:
            ...
            logger.info("[INSTAGRAM] Session not in memory — using demo video fallback")
            demo_filenames = ["veoplz_eng.MP4", "veoplz_eng.mp4", "veoplz_subtitled_source.mp4"]
            demo_dirs = [
                Path(__file__).resolve().parents[3] / "frontend" / "public" / "demo-assets",
                Path.cwd().parent / "frontend" / "public" / "demo-assets",
        ...
        file_size = Path(video_path).stat().st_size
        logger.info("[INSTAGRAM] Step 1: Video resolved (%d bytes)", file_size)

        # ── Step 2: Upload to GCS for a public URL ──
        # Instagram needs a publicly fetchable video URL.
        # Try GCS_BUCKET_NAME first, fall back to Firebase default bucket.
        import uuid as _uuid

        gcs_bucket_name = os.getenv("GCS_BUCKET_NAME", "")
        blob = None
        blob_name = f"ig-tmp/{_uuid.uuid4().hex}.mp4"

        if gcs_bucket_name:
            from google.cloud import storage as gcs_storage
            gcs_client = gcs_storage.Client(project=os.getenv("GCP_PROJECT", "amaran-ai"))
            ...
            fb_bucket_name = os.getenv("GCP_PROJECT", "amaran-ai") + ".firebasestorage.app"
            bucket = fb_storage.bucket(fb_bucket_name)
            blob = bucket.blob(blob_name)

        blob.upload_from_filename(video_path, content_type="video/mp4")
        # Bucket has allUsers:objectViewer IAM — public URL works directly
        public_url = blob.public_url

        logger.info("[INSTAGRAM] Step 2: Uploaded to GCS → %s", public_url)
        ...
        # Detect token type: IGAA tokens use graph.instagram.com, EAA tokens use graph.facebook.com
        if ig_access_token.startswith("IGAA"):
            graph_base = "https://graph.instagram.com/v21.0"
        else:
            graph_base = "https://graph.facebook.com/v21.0"
        logger.info("[INSTAGRAM] Using API base: %s", graph_base)
        async with _aiohttp.ClientSession() as session:
            ...
            if "id" not in create_data:
                ...
                raise HTTPException(status_code=502, detail=f"IG container creation failed: {error_msg}")

            container_id = create_data["id"]
            logger.info("[INSTAGRAM] Step 3: Container created → %s", container_id)
            ...
            for i in range(max_polls):
                ...
                logger.info("[INSTAGRAM] Step 4: Poll %d — status=%s", i + 1, status_code)

                if status_code == "FINISHED":
                    break
                elif status_code == "ERROR":
                    raise HTTPException(status_code=502, detail="IG video processing failed")
            else:
                raise HTTPException(status_code=504, detail="IG video processing timed out")

            # ── Step 5: Publish ──
            publish_resp = await session.post(
            ...
            if "id" not in publish_data:
                ...
                raise HTTPException(status_code=502, detail=f"IG publish failed: {error_msg}")
        ...
        # ── Step 6: Cleanup GCS temp object ──
        try:
            blob.delete()
            logger.info("[INSTAGRAM] Cleaned up GCS temp blob")
        except Exception:
            pass

        elapsed = time.time() - t0
        logger.info("[INSTAGRAM] === Published successfully in %.1fs === permalink=%s", elapsed, permalink)
        ...
        logger.error("[INSTAGRAM] Failed to publish: %s", e, exc_info=True)
        # Cleanup GCS blob on error
        try:
            if "blob" in dir() and blob:
                blob.delete()
        except Exception:
            pass
