...
export function PageSocial() {
  ...
  function handleShareToSocial(target: "whatsapp" | "instagram" | "tiktok" | "facebook" | "x") {
    const caption = socialOutput?.captions?.[selectedCaptionIdx]?.caption ?? ""
    const hashtags = socialOutput?.hashtags?.hashtag_string ?? ""
    const text = [caption, hashtags].filter(Boolean).join("\n\n")
    navigator.clipboard.writeText(text).catch(() => {})
    setCopiedField("share-" + target)
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground mr-1">
            {copiedField?.startsWith("share-") ? "Caption & hashtags copied!" : "Post to:"}
          </span>
          <Button variant="outline" size="sm" className="h-8 gap-1.5 hover:border-green-500/30" onClick={() => handleShareToSocial("whatsapp")}>
            <MessageCircle className="size-4 text-green-400" /> WhatsApp
          </Button>
          <Button variant="outline" size="sm" className="h-8 gap-1.5 hover:border-pink-500/30" onClick={() => handleShareToSocial("instagram")}>
            <Instagram className="size-4 text-pink-400" /> Instagram
          </Button>
          <Button variant="outline" size="sm" className="h-8 gap-1.5 hover:border-foreground/30" onClick={() => handleShareToSocial("tiktok")}>
