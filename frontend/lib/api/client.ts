...
/**
 * Publish the stitched video directly to YouTube.
 * Requires YOUTUBE_* credentials configured on the backend.
 */
export async function publishToYouTube(
  request: YouTubePublishRequest
): Promise<YouTubePublishResponse> {
  return fetchApi<YouTubePublishResponse>("/social/publish/youtube", {
    method: "POST",
    body: JSON.stringify(request),
  });
}
...
/**
 * Publish the stitched video as an Instagram Reel.
 * Requires INSTAGRAM_* credentials configured on the backend.
 */
export async function publishToInstagram(
  request: InstagramPublishRequest
): Promise<InstagramPublishResponse> {
  return fetchApi<InstagramPublishResponse>("/social/publish/instagram", {
    method: "POST",
    body: JSON.stringify(request),
  });
}
